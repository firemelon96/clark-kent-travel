...
export const AccomContactDetail = () => {
  ...
  const onSubmit = (values: z.infer<typeof contactFormSchema>) => {
    const newValues = {
      ...
      date: `${format(new Date(eachParams.from), "EEEE, MMM d")} - ${format(new Date(eachParams.to), "EEEE, MMM d")}`,
      count: +eachParams.participants,
      type: eachParams.type,
      title: eachParams.title,
      numOfNights: +eachParams.numOfNights,
    };

    startTransition(() => {
      AccomReservation(newValues)
        .then((data) => {
